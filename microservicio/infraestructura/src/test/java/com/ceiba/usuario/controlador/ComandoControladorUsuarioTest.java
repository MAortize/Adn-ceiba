...
class ComandoControladorUsuarioTest {
    ...
    void deberiaActualizarUnUsuario() throws Exception{
        // arrange
        Long id = 1L;
        ComandoUsuario usuario = new ComandoUsuarioTestDataBuilder().conNombre("prueba").build();
        // act - assert
        mocMvc.perform(put("/usuarios/{id_usuario}",id)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(usuario)))
                .andExpect(status().isOk());
    }
    ...
    void deberiaEliminarUnUsuario() throws Exception {
        ...
        Long id = 2L;
        // act - assert
        mocMvc.perform(delete("/usuarios/{id_usuario}",id)
                .contentType(MediaType.APPLICATION_JSON)
